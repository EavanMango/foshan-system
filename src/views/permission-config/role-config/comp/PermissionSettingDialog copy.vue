    ...
    <div class="dialog-content">
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="区域权限" name="area"
          ><SelectTree
            :data="areaData"
            :targetKeys="areaDataKeys"
            ref="areaTree"
          ></SelectTree
        ></el-tab-pane>
        <el-tab-pane label="厂站权限" name="station">
          <BaseTransfer
            v-if="activeName === 'station'"
            :data="stationData"
            :targetKeys="stationDataKeys"
            @change="keys => handleChange('stationDataKeys', keys)"
          ></BaseTransfer>
        </el-tab-pane>
...
// import { API_GROUP } from '@/api/group-config';
...
export default {
  ...
  methods: {
    ...
    async initSelectKeys(data) {
      ...
      const { substationIds, regionIds, routeIds, resourceIds } = res.data;
      this.menuDataKeys = (resourceIds || []).map(d => '' + d);
      this.areaDataKeys = (regionIds || []).map(d => '' + d);
      this.stationDataKeys = (substationIds || []).map(d => '' + d);
    ...
    async getAreaList() {
      const res = await API_GROUP.AppGroupRegionQueryAll({ namePattern: null });
      if (res.code !== 200) return;
      this.areaData = translateTreeData(res.data);
    },
    async getStationList() {
      const res = await API_GROUP.AppGroupSubstationQueryAll({
        idOrCnNamePattern: null,
        ofRegions: null,
      });
      if (res.code !== 200) return;
      this.stationData = translateTreeData(res.data);
    },
    async getRouteList() {
    ...
    handleBindArea() {
      API.RebindRegions({
        ids: this.$refs.areaTree.getSelectKeys().map(d => +d),
        roleId: this.editorDataCopy.id,
      });
    },
    handleBindStation() {
      API.RebindSubstations({
        ids: this.stationDataKeys.map(d => +d),
        roleId: this.editorDataCopy.id,
      });
    },
    handleBindRoute() {
